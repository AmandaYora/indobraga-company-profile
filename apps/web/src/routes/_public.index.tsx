...
function HomePage() {
  ...
  const displayedPortfolios = data?.featured_portfolios ?? portfolios;
  const displayedMachines = data?.facilities_summary.machines ?? machines;
  const displayedPrinting = data?.facilities_summary.printing_capacities ?? printingCapacity;
  const displayedServices = data?.facilities_summary.services ?? services.map((name) => ({ name }));
  const displayedNews = data?.latest_news ?? news;
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    {"desc" in p ? p.desc : p.short_description}
                  </p>
                </div>
              </article>
            ))}
                ...
                <div className="p-5">
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-muted-foreground">
                      {formatDateId("date" in n ? n.date : (n.published_at ?? ""), "long")}
                    </span>
                  </div>
