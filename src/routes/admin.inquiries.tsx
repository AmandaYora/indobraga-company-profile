...
function I() {
  return (<><PageTitle title="Contact Inquiries" desc="Daftar inquiry dari form kontak public website." /><Card className="overflow-hidden p-0"><table className="w-full text-sm"><thead className="bg-secondary text-xs uppercase tracking-wider text-muted-foreground"><tr><th className="p-4 text-left">Pengirim</th><th className="p-4 text-left">Pesan</th><th className="p-4 text-left">Status</th><th className="p-4 text-left">Tanggal</th></tr></thead><tbody className="divide-y divide-border">{items.map((i) => (<tr key={i.id} className="hover:bg-secondary/40"><td className="p-4"><p className="font-semibold">{i.name}</p><p className="text-xs text-muted-foreground">{i.email} · {i.phone}</p></td><td className="p-4 max-w-md text-muted-foreground line-clamp-2">{i.message}</td><td className="p-4"><StatusBadge status={i.status} /></td><td className="p-4 text-muted-foreground">{new Date(i.date).toLocaleDateString("id-ID")}</td></tr>))}</tbody></table></Card></>);
}
